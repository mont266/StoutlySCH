  ...
  const fetchContent = useCallback(async () => {
    ...
    try {
      const [ratingsRes, postsRes] = await Promise.all([
        supabase
          .from('ratings')
          .select('id, created_at, quality, message, image_url, like_count, comment_count, price, is_private, pubs(name), profiles:profiles!ratings_user_id_fkey(username, avatar_id)')
